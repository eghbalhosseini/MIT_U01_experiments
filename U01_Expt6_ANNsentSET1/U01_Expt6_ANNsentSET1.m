...
subject_output_file = [rootDir dataDir expt_name '_' subjID '_list' num2str(list) '.csv'];
...
if exist(subject_output_file,'file')
    ...
    stimuli_idx = all_stimuli.list == list;
...
for j =start:NUM_STIMULI
    ...
    %wait for audio to finish (will be showing a speaker on the screen)
    WaitSecs(timeLimit);
    ...
    output = table(final_list, trial, trial_onset, final_audio_filename, final_condition, final_audio_transcript, pressed_space_to_continue, trial_completed,resume_number);
